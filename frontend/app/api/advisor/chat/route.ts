...
import { runAdvisorChat } from "@/lib/ai/groqAdvisor";
...
export async function POST(request: Request) {
  ...
  try {
    const reply = await runAdvisorChat(JSON.parse(payload));
    return NextResponse.json({ reply, source: "groq-fallback" });
  } catch (error) {
    console.error("[advisorChatFallback] Failed:", error instanceof Error ? error.message : "unknown");
