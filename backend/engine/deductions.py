...
- Declared amounts are clamped at zero on entry — a negative EPF/donation/
  interest value (bad input/parser error) can no longer reduce other
  deductions or income.
- round2 (half-up) replaces round(x, 2) banker's rounding, matching the
  statutory convention.
- 80G: the 50%-rate donation bucket is capped at 10% of the caller-supplied
  GTI proxy before the 50% rate is applied (the statutory qualifying limit
  for that category); the 100%-rate bucket (PM Relief Fund etc.) is
  uncapped. KNOWN APPROXIMATION: the proxy is GTI, not true "adjusted
  total income" (which excludes special-rate LTCG/STCG and other Chapter
  VI-A deductions) — same caveat as 80GG below. Over-allows slightly for
  filers with capital gains; flag for CA review, don't silently tighten
  without a golden test.
- 80GG: same ATI-proxy approximation as 80G above.
"""
...
def compute_deductions(
    ...
    adjusted_total_income: float = 0.0,  # GTI proxy for 80GG/80G caps
    ...
    # 80G — 100%-rate categories uncapped; 50%-rate categories capped at
    # 10% of the ATI proxy before the 50% rate is applied (statutory
    # qualifying limit). See module docstring for the ATI-proxy caveat.
    qualifying_limit = ruleset.rate_80g_qualifying_limit * clamp0(adjusted_total_income)
    capped_donations_50pct = min(clamp0(d.donations_50pct), qualifying_limit)
    deduction_80g = clamp0(d.donations_100pct) + (capped_donations_50pct * 0.50)
