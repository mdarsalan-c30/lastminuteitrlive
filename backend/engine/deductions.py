...
def _compute_80gg(rent_paid: float, adjusted_total_income: float) -> float:
    ...
    if rent_paid <= 0:
        return 0.0
    limit_rent_excess = max(0.0, rent_paid - 0.10 * adjusted_total_income)
    limit_25pct = 0.25 * adjusted_total_income
    return round(max(0.0, min(CAP_80GG_ANNUAL, limit_25pct, limit_rent_excess)), 2)
...
def compute_deductions(
    ...
    if salary.hra_received > 0:
        ...
        deduction_80gg = _compute_80gg(d.rent_paid_no_hra, adjusted_total_income)
    ...
    return {
        ...
        "deduction_80gg": round(deduction_80gg, 2),
